...
🧱 Linux Runlevels – മലയാളം വ്യാഖ്യാനവും കമാൻഡുകളും
Runlevel	Command	Purpose (ഉദ്ദേശ്യം)
0	init 0	Shutdown/Halt ചെയ്യുന്നു (കമ്പ്യൂട്ടർ ഓഫാക്കുന്നു)
1	init 1	Single user mode (root maintenance only)
2	init 2	Multi-user mode (network ഇല്ലാതെ)
3	init 3	Multi-user mode + NFS (network support)
4	init 4	Unused (Development-ൽ കസ്റ്റമൈസ് ചെയ്യാവുന്നതാണ്)
5	init 5	X11 Mode – Graphics Mode (GUI – Desktop)
6	init 6	Reboot command – system വീണ്ടും സ്റ്റാർട്ടാകും
